...
|| Five-phase pipeline shape with phase gates | Lensetek, prototype | Workflow policy | Prototype only | Workflow policy | **Evidence:** Phase structure embodied in the `skills/` module set and skill contracts. Only the framework/story gate separation is ratified; exact gate set is prototype- or profile-specific and not yet executed. |
...
|| Thread Pull design | Project-specific Dust & Ash material | Optional project-profile behavior | Prototype only | Optional project-profile behavior | **Evidence:** Designated Dust & Ash profile material in Dust & Ash project files; not ratified reusable-core architecture. |
...
|| World/character/plot iteration as a prototype workflow observation | Prototype | Creative core | Prototype only | Workflow policy | **Evidence:** Prototype does not impose a rigid one-way order; observable in the `skills/` module set and skill contracts. Existing prototype behavior; not a ratified workflow rule. |
